#include "OsloLaBr/OCLLaBr3SensitiveDetector.hh"
#include "OsloLaBr/LaBrGlobals.hh"
...
OCLLaBr3SensitiveDetector::OCLLaBr3SensitiveDetector(G4String name)
        :G4VSensitiveDetector(name), histoManager(0), detectorID(-1),
        tracecounter(2), HCID(-1)
{
    HCname = LABR_HITS_COLLECTION_NAME;
    collectionName.insert(HCname);
}

// Initialize HitsCollection and histogramming
OCLLaBr3SensitiveDetector::OCLLaBr3SensitiveDetector(MiniBallHistoManager* HM,
                                                     G4String name)
...
void OCLLaBr3SensitiveDetector::Initialize(G4HCofThisEvent* evnt)
{
    hitsCollection = new LaBrHitsCollection(SensitiveDetectorName, HCname);
...
/*G4bool OCLLaBr3SensitiveDetector::ProcessHits(G4Step* aStep,
                                              G4TouchableHistory*)
{
