...
OCLLaBr3::OCLLaBr3(MiniBallHistoManager *hm, std::string p_name)
    : histoManager( hm )
    , name( p_name )
{
...
void  OCLLaBr3::CreateSolids()
{
    ...
    if ( histoManager ){
        ...
        labrSD = new MiniBallSensitiveDetector(histoManager, name);
        SDman->AddNewDetector(labrSD);
        logicCrystal->SetSensitiveDetector(labrSD);
    }
...
void OCLLaBr3::Placement(G4int copyNo, G4VPhysicalVolume* physiMother, G4bool checkOverlaps)
{
	...
	G4Transform3D transDetector = transform;//G4Transform3D(rotation,translatePos);
	...
	if ( histoManager ){
        ...
        detID = labrSD->GetDetectorID();

        histoManager->AddToCryostat(CryostatID, detID);
    }
